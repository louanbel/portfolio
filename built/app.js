jQuery(function () {
    ...
    document.getElementsByTagName("footer")[0].innerHTML = `<p class="ml-4 pb-0">Louan Bélicaud © Copyright ${year}</p>`;
...
window.customElements.define("loading-bar", class extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
...
window.customElements.define("nav-custom", class extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
    <nav id="nav" class="flex-row justify-content-center listeBtn2">
        ...
        <button id="b6" class="btn-success" onclick="window.open('doc/CV-louan-belicaud-2022.pdf')">getCV()</button>
    </nav>
    `;
    }
});
